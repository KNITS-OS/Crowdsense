...
export function ImportCv() {
  ...
  const onImportExcel = (e: ChangeEvent) => {
    ...
    reader.onload = (event) => {
      ...
      //convert to array
      const fileData: Array<object> = XLSX.utils.sheet_to_json(workSheet, {
        header: 1,
      });
      const dataKeys = importCurriculumsTableColumns().map(
        (item) => item.accessor
      ) as string[];
      //removing header
      fileData.splice(0, 1);
  ...
  const onImportCurriculums = async (selectedCurriculums: ICandidate[]) => {
    const initWorkflow = selectedCurriculums.map((cv) => {
      return { ...cv, workflow: "null" };
    });
    setSaveSent(true);
    // @todo Switch from single element import to collection
    await importCandidates(initWorkflow[0] as ICandidate)
      .unwrap()
      .then(() => {
        const filteredData = filterTwoArraysByReqId(
          importedData,
          selectedCurriculums
        );
        setIsSuccess(true);
